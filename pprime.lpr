...
musí byt ve formatu ze na cislo[1] je nejmensi cifra
  ...
  var
    vysledek: cisloT;
  var
    delka, i, posledniNenul, porov, zb: integer;
  begin
    vysledek.delka := 0;
    vynuluj(vysledek);
    zb := 0;
    ...
    begin
      porov := c1.cislo[i+1] - c2.cislo[i+1] + zb;
      vysledek.cislo[i+1] := betterMod(porov, 10);
      if (vysledek.cislo[i+1] > 0) then
        posledniNenul := i+1;
      if (porov >= 0) then
        zb := 0
      else
  ...
  begin
    ...
    //writeln('zredukovano :', vysl.quot.cislo[n - t + 1]);
    //vypisCislo(otocCislo(c1));
    ...
    begin
      ...
      begin
           ...
           if(vysl.quot.cislo[i-t] > 9) then
           begin
             writeln('je vetsi ', c1.cislo[i+1]*10, '+', c1.cislo[i], ' div ', c2.cislo[c2.delka]);
           end;
      end;
      ...
      begin
           writeln(longTemp, ' ', longTemp2);
           vysl.quot.cislo[i-t] := vysl.quot.cislo[i-t]-1;
           longTemp := vysl.quot.cislo[i-t]*(c2.cislo[c2.delka]*10 + c2.cislo[t]);
      end;
      if(vysl.quot.cislo[i-t] > 9) then
      begin
           writeln('XXXX VETSI ', vysl.quot.cislo[i-t]);
      end;
      vynasobeneDeseti := vynasobDeseti(c2, i-t-1);
      temp.delka := 1;
      temp.cislo[1] := vysl.quot.cislo[i-t];
      //writeln('nasobim: ',vysl.quot.cislo[i-t], ' s c2*10^', i-t-1);
      temp := vynasob2(temp, vynasobeneDeseti, false);
      //writeln('vysle');
      //vypisCislo(otocCislo(temp));
      temp2 := secti(c1, vynasobeneDeseti, false);
      c1 := odecti(c1, temp, false);
      if(c1.delka < 0) then
      begin
        c1 := temp2;
        writeln('pouzivam temp2');
        vysl.quot.cislo[i-t] := vysl.quot.cislo[i-t] - 1;
      end;
    end;
    for i := c1.delka downto 1 do
