...
type
  ...
  var
    i: integer;
  begin
    if(writeDelka) then writeln('delka : ', c.delka);
...
citelnyFormat := false;
  ...
  function vynasobDeseti(c1: cisloT; kolikrat: integer): cisloT; {dolni jsou nejmensi <=> citelnyFormat = false}
  var
    i: integer;
  begin
    if (kolikrat <= 0) then
...
function modulo(c1, c2: cisloT; jeCitelnyFormat : boolean): cisloT;
  ...
  begin
    ...
    begin
      ...
      begin
        q := q - 1;
        longTemp := q * (c2.cislo[c2.delka] * 10 +
          c2.cislo[c2.delka - 1]);
      end;
      vynasobeneDeseti := vynasobDeseti(c2, i - t - 1);
      temp.delka := 1;
      ...
      temp := vynasob(temp, vynasobeneDeseti, False);
      c1 := odecti(c1, temp, False);
      if (c1.isNegative = True) then c1 := odecti(vynasobeneDeseti, c1, False);
    end;
    if(jeCitelnyFormat) then c1 := otocCislo(c1);
    modulo := c1;
  end;
  ...
  begin
    ...
    if(jeCitelnyFormat) then vysl := otocCislo(vysl);
    square := vysl;
  end;

  function modular_pow(base, exponent, modulus: cisloT; jeCitelnyFormat : boolean): cisloT;
  var
    vysledek, dva: cisloT;
  var
...
begin
  ...
  //nahodne := stringToCislo('954449137391');
  FromTime := Now;
  x := modular_pow(nahodne, faktory.zbytek, p, true);
  ToTime := Now;
  DiffMinutes := MilliSecondsBetween(ToTime, FromTime);
  vypisCislo(p, true, 'p');
  vypisCislo(faktory.zbytek, true, 'zbytek');
  vypisCislo(nahodne, true, 'nahodne');

  writeln('time: ', DiffMinutes, ' ms');
  vypisCislo(otocCislo(x), true, '');
